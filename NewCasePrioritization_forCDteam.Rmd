...
diff$Investigation_Addr_Street[diff$Investigation_Addr_Street == ""] <- "NONE"
diff$Investigation_Addr_Street[is.na(diff$Investigation_Addr_Street)] <- "NONE"
diff$Investigation_Addr_Street[exclusion_Addresses %in% diff$Investigation_Addr_Street] <- "NONE"


#select the fields from MDSS that are needed for the analysis
diff <- diff %>% select(Case_Status, Case_Disposition, Investigation_ID, Project_Description,
                        Referral_Date, Investigation_Addr_Street, Physician_Affiliation,
                        Lab_1_Spec_Collection_Date,Referral_Date_new, Investigation_Addr_City)
...
#match patient's address with facility address and physician affiliation
diff <- merge(diff, FacilityByAddress, by = "Investigation_Addr_Street", all.x = TRUE)
...
#Create ordering anf flagging columns
diff <- diff %>% mutate(
  orderIndex = case_when(Flag_Inpatient == 1 ~ 1,
                   Flag_Facilities == 1 ~ 2,
                   Referral_Date_new == format(Sys.Date()-1, "%m/%d") & Case_Status == "Confirmed" ~ 3,
                   Referral_Date_new == format(Sys.Date()-1, "%m/%d") & Case_Status == "Probable" ~ 4,
                   Referral_Date_new == format(Sys.Date(), "%m/%d") & Case_Status == "Confirmed" ~ 5,
                   Referral_Date_new == format(Sys.Date(), "%m/%d") & Case_Status == "Probable" ~ 6,
                   Referral_Date_new == format(Sys.Date()-2, "%m/%d") & Case_Status == "Confirmed" ~ 7,
                   Referral_Date_new == format(Sys.Date()-2, "%m/%d") & Case_Status == "Probable" ~ 8,
                   TRUE ~ 9),
  Prioritization =  case_when(Flag_Inpatient == 1 ~ "Inpatient",
              Flag_Facilities == 1 & (Facility_type == "NH" | Physician_type == "NH") ~ "NH",
              Flag_Facilities == 1 & (Facility_type == "LTCF" | Physician_type == "LTCF") ~ "LTCF",
...
diff_new <- diff %>% select(Case_Status, Investigation_ID, Created, Project_Description, Referral_Date,
                            Investigation_Addr_Street, Physician_Affiliation,
                            Lab_1_Spec_Collection_Date, Prioritization, orderIndex, Referral_Date_new,
                            Case_Disposition, Facility_name,Flag_Inpatient,Flag_Facilities)
